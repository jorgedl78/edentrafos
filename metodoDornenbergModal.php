<?php
	...
	if(!$idTransformador){//fue llamado desde la lista de analiis
		$rs=mysqli_query($cn,"SELECT  Metano,Acetileno,Hidrogeno, DATE_FORMAT(Fecha, '%d/%m/%Y') as Fecha, Comentario  FROM analisis WHERE idAnalisis=$idAnalisis");
	}
	else{//fue llamado desde 
		$rs=mysqli_query($cn,"SELECT   Metano,Acetileno,Hidrogeno, DATE_FORMAT(Fecha, '%d/%m/%Y') as FechaStr, Comentario  FROM analisis WHERE idTransformador=$idTransformador order by Fecha desc limit 1");
	}
...
#classbody{
	...
	background-image: url("imagenes/metodoDornenberg.png");
...
<div id="fondo"><img src="imagenes/metodoDornenberg.png"></div>
<div id="divTriangulo"><img src="imagenes/Triangulo.gif" width="55" height="49"  title="<?php echo $fecha.' - '.$comentario;?>"/></div>
<div><input type="button" value="Cerrar" class="boton" onclick="location.href='#'"></div>
</body>
</html>
